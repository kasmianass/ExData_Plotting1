# same operation as in plot 1 : reading data, namming and subsetting

power <- read.table("household_power_consumption.txt",skip=1,sep=";")
names(power) <- c("Date","Time","Global_active_power","Global_reactive_power",
                  "Voltage","Global_intensity","Sub_metering_1","Sub_metering_2",
                  "Sub_metering_3")
subpower <- subset(power,power$Date=="1/2/2007" | power$Date =="2/2/2007") # we could use %in% to do so

# transforming date and time with another code other than the one I used in the first and 2nd plot
